...
// an OIDC provider from its discovery document.
//
...
// needs out of the same document, so the provider is described entirely by
// OIDC_ISSUER_URL rather than partly by the issuer and partly by a compiled-in
// constant.
//
// Callers must treat a failure here as fatal. Falling back to a hardcoded
// provider is precisely the bug this replaces: it produces a deployment that
// reads its issuer from config, validates ID tokens against that issuer, and
// nonetheless sends users somewhere else to log in — which cannot ever succeed
// and fails with a provider-side error that points nowhere near the cause.
